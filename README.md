...
We decided to use the official NSGA-II KanGAL code to build the NSGA-III, A-NSGA-III and A$^2$-NSGA-III versions, because we think it is the smoothest way. Other implementations shared on the Internet start from scratch which increase the complexity level. Only the reference \cite{nsga3cpp} shows validation results for DTLZ problems. Furthermore, we decided to complement the explanation about adaptive reference points generation.
The intention of this paper is to build the most similar NSGA-III code compared with the official and share the code to the research community. We would like to apply it to resolve Cloud Radio Access Network (C-RAN) planning problems to reduce the power consumption in cellular networks. 
We will expect this paper contribute to many researchers to solve many questions with regard to the implementation of the original NSGA-III. 
## Instructions
### input_data folder:
