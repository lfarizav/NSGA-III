...
There are two approaches\footnote{MOOPs are widely used to solve problems with two or three objective functions. In contrast, MaOPs resolve more than three objective functions.} to solve MOOPs and MaOPs. The first one combines all objective functions in a weighted sum scalarization method, but weights are sensible to the noise and decision maker needs, difficult to tune even for people who are very familiar with a specific problem. Additionally, it is possible to select one objective and set the rest an arbitrary constraint value. Those methods return a unique solution instead of a set of them. Although the second approach is complex, it is more popular, because it determines a Pareto solution set or a set of solutions that are nondominated. The above is convenient for decision makers to analyze trade-offs in real engineering problems.
